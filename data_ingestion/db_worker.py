...
class DatabaseWorker:
    def __init__(self, shared_queue, storage="else"):
        self.shared_queue = shared_queue
        if storage == "aws":
            db_host = "arbitrage.c782myq2coht.us-east-2.rds.amazonaws.com"
            db_name = "Arbitrage"
            db_use = "postgres"
            db_password = os.getenv("DB_PASSWORD")
            self.uri = psycopg.conninfo.make_conninfo(host=db_host, dbname=db_name, user=db_use, password=db_password)
        else:
    ...
    async def insert_kalshi_events(self, events, cursor):
        ...
        sql_markets = """
            INSERT INTO kalshi_markets (source,event_id, market_id, market_type, title, open_time, close_time, expiration_time, status, result, rules_primary, can_close_early, early_close_condition, rules_secondary, subtitle, category) 
            VALUES ('Kalshi', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ...
            subtitle = COALESCE(EXCLUDED.subtitle, kalshi_markets.subtitle),
            category = COALESCE(EXCLUDED.category, kalshi_markets.category)
            """
    ...
    async def insert_polymarket_events(self, events, cursor):
        ...
        for event in events:
            ...
            if isinstance(tags, list) and len(tags) > 0:
                ...
                if isinstance(first_tag, dict):
                    label = first_tag.get("label", "Unknown")
                else:
                    label = str(first_tag)
            else:
                label = "Unknown"
            ...
            events_tuple.append(
                (event.get("id", None), event.get("title", None), event.get("description", None), label)
            )
            for market in event.get("markets", []):
                markets_tuple.append(
        ...
        event_sql = """
            INSERT INTO polymarket_events (source, event_id, title, description, category) VALUES ('Polymarket', %s, %s, %s, %s)
            ON CONFLICT (source, event_id) DO UPDATE SET
    ...
    async def insert_polymarket_prices(self, message, cursor):
        ...
        for change in changes:
            ...
            """, (message.get("market"), change.get("best_bid"), change.get("best_ask"), change.get("price"), ts))
    ...
    async def backfill_event_data(self, cursor):
        ...
        SET calc_exp_date = sub.max_date, status = sub.final_status
        FROM (
        SELECT event_id, MAX(close_time) as max_date,
        CASE 
        WHEN COUNT(*) FILTER (WHERE status IN ('true', 'active', 'True')) > 0 THEN 'true'
        ELSE 'false'
        END as final_status
