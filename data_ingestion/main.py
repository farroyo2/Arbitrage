...
async def main():
    ...
    polymarket_ws_climate = WebSocketOrderBook(shared_queue,
        MARKET_CHANNEL, url, asset_ids, auth, None, True, all_tokens[2], "Climate"
    )
    polymarket_ws_sports = WebSocketOrderBook(shared_queue,
        MARKET_CHANNEL, url, asset_ids, auth, None, True, all_tokens[0], "Sports"
    )
    polymarket_ws_politics = WebSocketOrderBook(shared_queue,
        MARKET_CHANNEL, url, asset_ids, auth, None, True, all_tokens[1], "Politics"
    )
    ...
    tasks = [
        ...
        asyncio.create_task(polymarket_ws_climate.run()),
        asyncio.create_task(polymarket_ws_sports.run()),
        asyncio.create_task(polymarket_ws_politics.run()),
