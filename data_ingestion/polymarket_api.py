...
class WebSocketOrderBook:
    def __init__(self, shared_queue, channel_type, url, data, auth, message_callback, verbose, tokens, category):
    ...
    async def run(self):
        while True:
            try:
                ...
                self.furl, ping_interval=20, ping_timeout=20, max_size= 10_000_000
                ) as ws:
                    self.ws = ws
    ...
    async def on_open(self, ws):
        try: 
                ...
                if self.channel_type == MARKET_CHANNEL:
                    ...
                    await ws.send(json.dumps(innit_msg))
    ...
    async def subscribe_to_tokens_ids(self, assets_ids):
        ...
        if self.channel_type == MARKET_CHANNEL:
            chunk_size = 50
            total_chunks = len(assets_ids) // chunk_size + 1
            print(f"Splitting {self.category}'s {len(assets_ids)} tokens into {total_chunks} batches...")

            for i in range(0, len(assets_ids), chunk_size):
                chunk = assets_ids[i:i+chunk_size]
                try:
                    await self.ws.send(json.dumps({"assets_ids": chunk, "operation": "subscribe", "custom_feature_enabled": True}))
                    await asyncio.sleep(0.5)
                except Exception as e:
                    print("Error in Polymarket subscribe_to_tokens_ids message:", e)
                    await asyncio.sleep(5)
